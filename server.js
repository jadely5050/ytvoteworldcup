...
// 접속했든 같은 순간에 같은 위치에 그려져 PGM/PVW/직접 연 탭이 어긋나지 않는다.
const SCROLL_ITEMS_MAX = 60;
const scrollItems = []; // { seq, ts, type:'chat'|'guide', author, text }
let scrollSeq = 0;
let scrollGuideCount = 0; // guideFrequency 개 채팅마다 안내문 1회 삽입용 카운터
let lastScrollItemAt = 0; // 유휴(채팅 없음) 시 안내문 하트비트 판단용
...
// 채팅이 뜸해 스크롤이 다 지나가고 비면 안내문으로 주기적으로 채운다(유휴 하트비트).
const SCROLL_IDLE_GUIDE_MS = 10000;
setInterval(() => {
  if (!scrollOn) return;
  const sc = config.poll.scroll;
  if (!sc.showGuideText) return;
  if (Date.now() - lastScrollItemAt < SCROLL_IDLE_GUIDE_MS) return;
  pushScrollItem("guide", "", sc.guideText);
}, 2500);
...
wss.on("connection", (ws) => {
  ...
  // 최근 2분 내 항목만 재전송 — 늦게 접속한 화면도 같은 ts 로 같은 위치에 그린다.
  const cutoff = Date.now() - 120000;
  ws.send(JSON.stringify({ type: "scrollHistory", data: scrollItems.filter((i) => i.ts >= cutoff) }));
});
